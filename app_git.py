...
def get_assigned_area(name_str):
    ...
    if "ミニオン" in name_str or "Minion" in name_str:
        return "ミニオン・パーク"
    if "呪術廻戦" in name_str or "4-D" in name_str or "コナン" in name_str or "Conan" in name_str or "シュレック" in name_str or "セサミストリート" in name_str or "フリーレン" in name_str or "Frieren" in name_str or "Hollywood" in name_str or "ハリウッド" in name_str or "The Ride" in name_str or "Backdrop" in name_str:
        return "ハリウッド・エリア / ニューヨーク・エリア"
...
SELECTABLE_RIDES = sorted(list(set([
    ...
    "シング・オン・ツアー", "名探偵コナン・ザ・ワールド", "呪術廻戦・ザ・リアル 4-D ～廻る時計台～", "葬送のフリーレン",
    "セサミストリート 4-D ムービー・マジック™", "シュレック 4-D アドベンチャー™", "プレイング・ウィズ・おさるのジョージ™",
    "エルモのゴーゴー・スケートボード", "エルモのバブル・バブル", "エルモのリトル・ドライブ",
    "ハローキティのカップケーキ・ドリーム", "ハローキティのリボン・コレクション",
    "モッピーのバルーン・トリップ", "ビッグバードのビッグトップ・サーカス", "フライング・スヌーピー",
    ...
    if st.button("🔄 情報を更新する"):
        ...
        if rides:
            ...
            for r in rides:
                ...
                # 【プレフィックス削除の誤作動対策】
                # 1. 既に完全な形でNAME_MAPにある場合はそのまま採用
                if raw_name_stripped in NAME_MAP:
                    display_name = raw_name_stripped
                # 2. ハリウッドドリームなどの独自ハイフン持ちを除外しつつプレフィックスをカット
                elif " - " in raw_name_stripped and not any(k in raw_name_stripped for k in ["Hollywood Dream", "Space Fantasy", "スペース・ファンタジー"]):
                    display_name = raw_name_stripped.split(" - ", 1)[1].strip()
                else:
                    display_name = raw_name_stripped
                    
                # 【ユーザー要望】期間限定版がある場合、通常版の「スペース・ファンタジー・ザ・ライド」はスキップ
